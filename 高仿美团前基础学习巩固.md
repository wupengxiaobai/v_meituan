...
- 使用
    ...
    //  使用, 在代码中直接操作 session 就可以将session数据保存到 redis 中了,如下
    ctx.session.count ++ 
    
    
    //  不操作session,保存数据到 redis
    ...
    //  我们访问该路由就可以存 fix 值为一个随机小数
    //  访问 redis 数据 `hget fix name` 即可访问 flx 的值 -> 随机小数
    ```
    +  查看保存在 redis 中的 session 数据
    ```js
    //  打开新窗口 -> redis-cli 进入 redis 服务
    //  `keys *` //   查看所有session的信息
    127.0.0.1:6379> keys *
    //  `get 某条数据` 查看该条数据相关信息
    127.0.0.1:6379> get mtprS-3n9JGyuS8lTDyRKgzdK1zp0MPjCCYv
    //  "{\"cookie\":{\"httpOnly\":true,\"path\":\"/\",\"overwrite\":true,\"signed\":true,\"maxAge\":86400000},\"count\":1}"
...
**概述** [Vue 通用应用框架](https://zh.nuxtjs.org/guide/installation)
...
- 路由&示例
    ...
    **在pages目录下创建的.vue文件 即配置路由, 直接to访问指定模块即可**
